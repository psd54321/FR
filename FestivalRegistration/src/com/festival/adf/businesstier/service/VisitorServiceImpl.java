...
public class VisitorServiceImpl implements VisitorFacade {
	...
	public void unregisterEvent(Visitor visitor, int eventid, int eventsessionid) {
		...
		try {
			...
			log.info("Exception is(class not found) :" + exception.getMessage());
		} catch (SQLException exception) {
			log.info("Exception is (sql):" + exception.getMessage());
		} catch (Exception exception) {
			log.info("Exception is (exception):" + exception.getMessage());
		}
	}
